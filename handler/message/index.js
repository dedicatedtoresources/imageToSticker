...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        case 'menuadmin':
            ...
            if (!isGroupAdmins) return client.reply(from, '[Admin Group Only]', id)
        ...
        case 'stiker': {
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                */
                ...
                if (!is.Url(url)) { await client.reply(from, '[Invalid Link]', id) }
                client.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? client.sendText(from, '[No Image]')
                    : client.reply(from, 'Here\'s your sticker\nYash')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'gifsticker': {
            ...
            if (is.Giphy(url)) {
                ...
                await client.reply(from, '[Use Giphy Only]', id)
            }
            break
        }
        // Video Downloader
        case 'tiktok':
            if (args.length !== 1) return client.reply(from, '[Wrong Format]', id)
            if (!is.Url(url) && !url.includes('tiktok.com')) return client.reply(from, '[Invalid Link]', id)
            await client.reply(from, `Processing........please wait..... \n\n${menuId.textDonasi()}`, id)
            ...
            }).catch(() => client.reply(from, '[Invalid Link]', id))
            break
        case 'ig':
        case 'instagram':
            if (args.length !== 1) return client.reply(from, '[Wrong Format]', id)
            if (!is.Url(url) && !url.includes('instagram.com')) return client.reply(from, '[Invalid Link]', id)
            await client.reply(from, `Processing........please wait..... \n\n${menuId.textDonasi()}`, id)
            downloader.insta(url).then(async (data) => {
                ...
                .catch((err) => {
                    console.log(err)
                    if (err === 'Not a video') { return client.reply(from, ' [Invalid Link]', id) }
        ...
        case 'meme':
            ...
            break
        case 'resi':
            if (args.length !== 2) return client.reply(from, '[Wrong Format]', id)
        ...
        case 'ceklok':
        case 'ceklokasi':
            if (!quotedMsg || quotedMsg.type !== 'location') return client.reply(from, '[Wrong Format]', id)
            ...
            const text = `*CHECK THE LOCATION OF THE SPREAD OF COVID-19 * \ nThe results of the inspection from the location you sent are *${zoneStatus.status}* ${zoneStatus.optional}\n\nInformasi lokasi terdampak disekitar anda:\n${data}`
        ...
        case 'kick':
            ...
            break
        case 'promote':
            if (!isGroupMsg) return await client.reply(from, ' [Group Only]', id)
            if (!isGroupAdmins) return await client.reply(from, ' [Admin Group Only]', id)
            if (!isBotGroupAdmins) return await client.reply(from, ' [Bot not Admin]', id)
            if (mentionedJidList.length != 1) return client.reply(from, ' [Wrong Format, Only 1 user]', id)
            if (groupAdmins.includes(mentionedJidList[0])) return await client.reply(from, ' [Bot is Admin]', id)
            if (mentionedJidList[0] === botNumber) return await client.reply(from, '[Wrong Format]', id)
        ...
        case 'demote':
            ...
            await client.sendTextWithMentions(from, `Request accepted, removed @${mentionedJidList[0].replace('@c.us', '')} as admin.`)
            break
        case 'bye':
            if (!isGroupMsg) return client.reply(from, '[Group Only]', id)
            if (!isGroupAdmins) return client.reply(from, '[Admin Group Only]', id)
